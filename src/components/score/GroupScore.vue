  ...
  <div class="page score-page" v-if="pageData">
    <div class="rank-ct">
      <div class="total-rank-ct">
        <div class="total-rank-bg">
          <div class="total-rank-num">{{pageData.TotalRank}}</div>
          <p>总排名</p>
          <p>打败<i class="total-rank-rate">{{pageData.TotalPer}}</i></p>
        </div>
      </div>
      <ul class="rank-list">
        <li class="li-quarter">
          <div class="num">{{pageData.QuarterRank}}</div>
          <p>季排名</p>
          <p>打败{{pageData.QuarterPer}}</p>
        </li>
        <li class="li-month">
          <div class="num">{{pageData.MonthRank}}</div>
          <p>月排名</p>
          <p>打败{{pageData.MonthPer}}</p>
        </li>
        <li class="rank-list-no-bg li-day">
          <div class="num">{{pageData.DayRank}}</div>
          <p>日排名</p>
    ...
    <div class="score-ct">
      <ul class="group-score-list">
        <li>
          <div>
            <div class="total-score">{{pageData.TotalScore}}</div>
            <p>总学分</p>
        ...
        <li class="rank-list-no-bg">
          <div>
            <div class="month-score">{{pageData.MonthScore}}</div>
            <p>月度分</p>
      ...
      <div class="score-detail-ct">
        <div class="score-item">
          <span class="signin-ct"><label>签到学分：</label><i>{{pageData.SignScore}}</i></span>
          <span class="signin-month-ct"><label>月度签到分：</label><i>{{pageData.MonthSignScore}}</i></span>
        </div>
        <div class="score-item">
          <span class="course-ct"><label>课程学分：</label><i>{{pageData.CourseScore}}</i></span>
          <span class="course-month-ct"><label>月度课程分：</label><i>{{pageData.MonthCourseScore}}</i></span>
        </div>
        <div class="score-item">
          <span class="others-ct"><label>其他学分：</label><i>{{pageData.OtherScore}}</i></span>
          <span class="others-month-ct"><label>月度其他分：</label><i>{{pageData.MonthOtherScore}}</i></span>
        </div>
      </div>
    </div>
  </div>
...
<script>
  import {getGroupScore} from '../../api/score'
  export default{
    name: 'GroupScore',
    data(){
      return {
